...
def _controlla(db: Session) -> int:
    """Nucleo testabile: restituisce il numero di email inviate."""
    ...
    for lavoro in lavori_in_scadenza:
        # data_scadenza_pagamento è FlexDate → già un oggetto date, non una stringa
        scadenza = lavoro.data_scadenza_pagamento
        if not scadenza:
            continue
